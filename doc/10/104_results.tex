...
Als technische Grundlage wurde die öffentlich verfügbare 3D-Bearbeitungssoftware Blender verwendet, um \gls{ifc}-Modelle zu laden und zu bearbeiten (vgl. Abb. \ref{fig:104_blender-ifc-viewer}).
...
die Verbesserung der Anfragearchitektur sowie die Integration einer primären Datenquelle ohne externe Softwareabhängigkeiten vorsah,
...
sodass statt einer breiten Abdeckung von Modellbearbeitungsfunktionen wurden nur noch Werkzeuge zur Wissensgewinnung bereitgestellt werden. \\
...
Auf Basis der nach der Entwicklung des Prototypen verfeinerten Entwicklungsplans wurde der lauffähige Prototyp entwickelt. 
...
Der in der Abbildung dargestellte \gls{ifc}-Viewer diente als Referenz, verfügte jedoch nicht über einen direkten Bezug zwischen den dem \gls{llm} zur Verfügung stehenden Daten und den abgebildeten Modellen im Viewer.
...
\begin{table}[!htb]
    \centering
    \caption[Definition von Skills im Skills-Werkzeug]{Englische Definition von Skills, welche vom Skills-Werkzeug bereitgestellt werden}
...
\begin{table}[!htb]
    \centering
    \caption[Definition von Werkzeugen]{Englische Definition von Werkzeugen, welche dem \gls{llm} bereitgestellt werden}
...
Die Ergebnisse, welche unter Verwendung der Anwendung erzeugt wurden, sind im folgenden Abschnitt dargestellt und analysiert.
...
\textbf{Vergleichen Sie, ob alle Ebenen (IfcBuildingStorey) in den Modellen gleich benannt und die gleiche Höhe besitzen.}
...
Probanden der Gruppe B mit minimalem Grundwissen nutzten Formulierungen, welche näher an dem im Systemprompt (vgl. Kap. \nameref{sec:appendix-systemprompt}) 
...
\end{tikzpicture}
\caption[Gesamtergebnisse der \gls{llm}-Auswertung]{Die Analyse ergab, dass von 41 Ausgaben des \gls{llm} 7.3\% abgebrochen wurden, 51.2\% der Ausgaben erzeugt und als inkorrekt eingestuft wurden, 36.6\% der Ausgaben erzeugt und als korrekt eingestuft wurden, und 4.9\% der Fälle keine Ausgabe erzeugten.}

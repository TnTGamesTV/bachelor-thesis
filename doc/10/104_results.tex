...
Dieses Kapitel führt die Ergebnisse vor, die durch der in Kapitel \ref{chap:methods} beschriebenen Methodik erzielt wurden.
...
Des Weiteren wurde die Definition des Systemprompts, sowie die Anfragearchitektur im Entwicklungsplan als Kernelemente identifziert und ausgebaut.
...
Der in der Abbildung dargestellte \gls{ifc}-Viewer diente als Referenz, verfügte jedoch nicht einen direkten Bezug zwischen den dem \gls{llm} zur Verfügung stehenden Daten und den abgebildeten Modellen im Viewer.
...
\begin{figure}[htb]
	...
	\caption[Lauffähiger Prototyp mit Modellauswahl]{Bevor das Chatfenster geöffnet wird, wird im  lauffähigen Prototypen die Modellauswahl angezeigt. Die gleichzeitige Auswahl mehrere Fachmodelle war innerhalb eines Projektes möglich.}
...
Während im vorherigen Kapitel die Ergebnisse aus Sicht der Probanden präsentiert wurden, wird in diesem exklusiv die Analyse der \gls{llm}-Ausgaben betrachtet.
...
\end{tikzpicture}
\caption[Fehlerergebnisse der \gls{llm}-Auswertung]{Von den vorherigen 21 fehlerhaften Ausgaben schlüsselen sich die Gründe wie folgt auf: 9.5\% der Fehler wurden durch die Nutzung einer falschen IFC-Entität verursacht, 23.8\% durch die Nutzung einer falschen Property oder Pset, 14.3\% durch die Nutzung eines falschen Werkzeugs, 23.8\% durch falsch aufgebaute SQL-Abfragen, und 28.6\% durch sonstige Fehler, welche nicht Zusammenhang mit dem \gls{llm} stehen.}
...
Bei der Verwendung der dem \gls{llm} zur Verfügung stehenden Abfragewerkzeuge (Query-Werkzeug) wählte das dieses in der Regel die Injektion von Zeilenumbrüchen ("\textbackslash{n}") 
...
die aus der Anfrage des Probanden abgeleitet war. Dies führte dazu, dass in Kombination mit einer direkten Suche Properties nicht korrekt gefunden wurden oder frühzeitig ein Ergebnis erzeugte,
...
sodass die erzeugte Raumliste, sowie berechneten Flächenwerte nicht korrekt waren. Dennoch wurden die geladenen Modelle korrekt verwendet,

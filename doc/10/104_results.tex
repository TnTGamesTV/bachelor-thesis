...
Der Skill \enquote{ask-user} unterstützt das \gls{llm} bei notwendigen Rückfragen an den Nutzer und beschreibt, wie Fragen strukturiert werden sowie in welchen Situationen Rückfragen gestellt werden sollen.
Im Skill \enquote{query} ist das datenbankbezogene Wissen gebündelt.
...
Der Skill \enquote{ifc} beschreibt den \gls{ifc}-Standard in kompakter Form und erläutert den Aufbau der verfügbaren \gls{ifc}-\gls{json}-Strukturen einschließlich der nutzbaren Abfrageparameter.
...
Das SkillsModule benutzt dabei das gleichnamige Werkzeug \enquote{skill}, um dem \gls{llm} die in Kapitel \ref{sec:app-skillsmodule} beschriebenen Funktionalitäten des inkrementellen Wissensmanagements bereitzustellen. \\
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
    \caption{Die Anzahl der Probanden, welche an Interviews partizipiert haben, beläuft sich auf drei Teilnehmer der Gruppe A und drei Teilnehmer der Gruppe B.}
...
Wie in der Abbildung \ref{fig:104_results-ai-errors} zu erkennen ist, gab es neben Fehlern, die außerhalb des Einflussbereichs des \gls{llm} lagen (\enquote{Sonstige Fehler}), 
primär Probleme mit der Datenbankabfrage (\enquote{SQL-Abfrage falsch aufgebaut}) sowie der Suche nach Properties (\enquote{Falsche Property/Pset genutzt}). 
Bei der Verwendung der dem \gls{llm} zur Verfügung stehenden Abfragewerkzeuge (Query-Werkzeug) wählte dieses in der Regel die Injektion von Zeilenumbrüchen (\enquote{\textbackslash{n}}) 
sowie Semikola (\enquote{;}) in den SQL-Abfragen an die Datenbank, obwohl dies im Systemprompt explizit verboten wurde. 

...
\hyphenation{Vor-ge-hens-wei-se}
\hyphenation{be-schrie-ben}
\hyphenation{Phä-no-me-ne}
\hyphenation{Kon-zep-ten}
\hyphenation{Er-klä-rung}
...
\usepackage[german,english]{babel}       % language: german and english, english is main language, german is used in the german abstract and the acknowledgements 
% \usepackage[T1]{fontenc}
...
\begin{document}
    ...
    \include{./doc/00/001_title}
    \include{./doc/00/002_task}
    \include{./doc/00/003_abstract}

    \clearpage

    \setstretch{1.25}

    % set glossary commands to noindex and disable hyperlinks
    % \GlsXtrSetDefaultGlsOpts{local,noindex}
    % \glsdisablehyper
    ...
    % \glsenablehyper
    % \GlsXtrSetDefaultGlsOpts{}   % set glossary commands to default and enable hyperlinks
    \glsresetall % reset glossary entries usage
    \cleardoublepage

    \setcounter{page}{1}
    \pagenumbering{arabic}
    \hideactivechapterheader
    \include{./doc/10/101_intro} % Introduction 
    \showactivechapterheader
    \include{./doc/10/102_state-of-the-art} % State of the art
    \include{./doc/10/103_methods} % Methods
    \include{./doc/10/104_results} % Results
    \include{./doc/10/105_discussion} % Discussion & Outlook
